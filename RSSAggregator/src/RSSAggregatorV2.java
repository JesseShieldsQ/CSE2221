 ...
 *
 */
public final class RSSAggregatorV2 {

    /**
     * Private constructor so this utility class cannot be instantiated.
     */
    private RSSAggregatorV2() {
    }
    ...
    private static void processItem(XMLTree item, SimpleWriter out) {
        ...
        if (dateLocation >= 0 && item.child(dateLocation).numberOfChildren() > 0) {
        ...
        if (item.child(titleLocation).numberOfChildren() > 0) {
            if (item.child(linkLocation).numberOfChildren() > 0) {
                out.println("<td>\n<a href=\"" + item.child(linkLocation).child(0) + "\">"
                        + item.child(titleLocation).child(0) + "</a>\n</td>");
            } else {
                out.println("<td>" + item.child(titleLocation).child(0) + "</td>");
            }
        } else if (item.child(descriptionLocation).numberOfChildren() > 0) {
            if (item.child(linkLocation).numberOfChildren() > 0) {
                out.println("<td>\n<a href=\"" + item.child(linkLocation).child(0) + "\">"
                        + item.child(descriptionLocation).child(0) + "</a>\n</td>");
            } else {

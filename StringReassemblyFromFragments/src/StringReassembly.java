...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        //Gets substring of string1 from the beginning to the start of the string
        //overlap
        String firstCut = str1.substring(0, str1.length() - overlap);
        //Combines the first string without the overlap text with the second string
        //and returns it
        return firstCut + str2;
    }
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet, String str) {
        ...
        boolean strContainsSubStr = false;
        boolean elementContainsStr = false;

        //Checks for substrings between every element of strSet and str
        //and enables a flag if either is a subset of the other.
        for (String element : strSet) {
            //If element is in string, that means that we need to remove that
            //element and then add string, so we store that element in a temporary
            //set
            if (str.contains(element)) {
                strContainsSubStr = true;
                temp.add(element);
            }
            //If str is a subset of any element, we make our flag true
            if (element.contains(str)) {
                elementContainsStr = true;
            }
        }
        //If any element is a substring of string and doesn't exist in the set,
        //we remove that element and add string
        if (strContainsSubStr && !strSet.contains(str)) {
            strSet.remove(temp);
            strSet.add(str);
            //If str isn't in strSet and str isn't a substring of any element,
            //add str to strSet
        } else {
            if (!strSet.contains(str) && !elementContainsStr) {
                strSet.add(str);
            }
        }
    }
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        //loops until at the end of the stream
        while (!input.atEOS()) {
            //reads a line of text and then adds it our return set, avoiding
            //substrings
            String inputLine = input.nextLine();
            addToSetAvoidingSubstrings(output, inputLine);
        }
    ...
    private static int bestOverlap(Set<String> strSet, String[] bestTwo) {
        ...
        while (strSet.size() > 0) {
            ...
            for (String str1 : strSet) {
                 ...
                 */
                if (str0 == str1) {
                    overlapFrom0To1 = str1.length();
                } else {
                    overlapFrom0To1 = overlap(str0, str1);
                 ...
                 */
                if (str0 == str1) {
                    overlapFrom1To0 = str1.length();
                } else {
                    overlapFrom1To0 = overlap(str1, str0);
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        //loops through the input text and for each character, prints a line separator
        //if the char is '~', printing the character otherwise
